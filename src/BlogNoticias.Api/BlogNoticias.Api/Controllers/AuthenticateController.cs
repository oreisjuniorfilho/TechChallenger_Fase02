...
using BlogNoticias.Api.Models.Auth;
        ...
        public async Task<IActionResult> CreateUserAsync([FromBody] CreateUserModel model)
        {
            ...
            if (userExists is not null)
                return StatusCode(
                    ...
                    new ResponseModel { Success = false, Message = "Usuário já existe na base de dados!" }
                );
            ...
            if (!result.Succeeded)
                return StatusCode(
                    StatusCodes.Status500InternalServerError,
                    new ResponseModel { Success = false, Message = "Erro ao criar um novo usuário!" }
                );
            ...
            return Ok(new ResponseModel { Message = "Usuário foi criado com sucesso pela aplicação!" });
        }
